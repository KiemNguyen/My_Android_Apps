package net.kiemnguyen.top10songs;
...
public class MainActivity extends Activity {
		...
		btnParse.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (operationStatus) {
					...
					//Take all data in the Application object one at a time and create entry in the ListView.
					ArrayAdapter<Application> adapter = new ArrayAdapter<Application>(MainActivity.this, R.layout.list_item, allApps);
					lvApp.setVisibility(lvApp.VISIBLE);
	...
	private class DownloadData extends AsyncTask<String, Void, String> {
		...
		//Create downloadXML method
		private String downloadXML(String theUrl) throws IOException {
			//Set how many character we can download at a time from the file
			...
			try {
				URL appleRSS = new URL(theUrl);
				HttpURLConnection myConnection = (HttpURLConnection) appleRSS.openConnection();
				myConnection.setReadTimeout(10000);
				myConnection.setConnectTimeout(15000);
				myConnection.setRequestMethod("GET");
				myConnection.setDoInput(true);
				int response = myConnection.getResponseCode();
				Log.d("DownloadXML", "The resoinse return is: " + response);
				...
				try {
					...
					//Show where is the crash
					e.printStackTrace();
					return null;
				}
				//Close InputStream whether there is an error or not
			} finally {
				if (is != null)
					is.close();
			}
